...
2. **Sổ câu sai + Spaced repetition:** Mọi câu practice/mock sai → ghi lại (đề, đáp án đúng, **lý do mình sai**). Ôn lại theo mốc **1 ngày → 3 ngày → 7 ngày**. Viết file phân tích 6 mục trong `KAFKA/questions/` theo đúng format `aws-saa-c03-analysis-format.md` của repo.
...
4. **Ngưỡng đăng ký thi thật — CHỈ đặt lịch khi ĐỦ CẢ 4:**
   - ✅ **≥ 3 bộ practice CCDAK KHÁC NHAU đạt ≥ 80%** (ổn định, không phải may mắn).
...
- KIP-932: early access 4.0 → preview 4.1 → **GA 4.2**; `share.acquisition.lock.duration.ms` **30 s**.
...
> 💡 **Tận dụng repo này:** mỗi câu practice sai → viết 1 file phân tích trong `KAFKA/questions/` theo format 6 mục (xem `aws-saa-c03-analysis-format.md`), đặt tên `KAFKA-NNNN.md`.
...
- [ ] **≥ 3 bộ practice CCDAK khác nhau ≥ 80%**.
- [ ] Review 100% câu sai; viết file phân tích `KAFKA/questions/KAFKA-NNNN.md`.
