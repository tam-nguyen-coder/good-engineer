...
> **Nguồn (official):** https://kafka.apache.org/43/operations/tiered-storage/ (mục 6.11 Tiered Storage) · KIP gốc: https://cwiki.apache.org/confluence/display/KAFKA/KIP-405%3A+Kafka+Tiered+Storage (cwiki không crawl được trong phiên này — phần KIP tổng hợp từ docs)
> **Tuần:** 8 — Observability & Operations · **Loại:** Apache Kafka Docs + KIP
...
- **Ý tưởng**: 2 tầng lưu trữ — **local tier** (disk broker, phục vụ tail read qua page cache) và **remote tier** (S3/HDFS/GCS…) chứa các **segment đã đóng** (không phải active segment). Tách **compute khỏi storage**, retention dài gần vô hạn, giảm disk/broker, **recovery & reassignment nhanh hơn** vì chỉ copy local data.
- **Broker config**: `remote.log.storage.system.enable=true` (bật tính năng, mặc định false); **bắt buộc** `remote.log.storage.manager.class.name` + `remote.log.storage.manager.class.path` (Kafka **không** ship `RemoteStorageManager` sẵn — dùng plugin như Aiven `tiered-storage-for-apache-kafka`, hoặc `LocalTieredStorage` test jar để thử). `RemoteLogMetadataManager` mặc định lưu metadata trong **internal topic `__remote_log_metadata`**; cần `remote.log.metadata.manager.listener.name`.
- **Topic config**: `remote.storage.enable=true` (mặc định false) + `local.retention.ms` / `local.retention.bytes` (thời gian/dung lượng giữ **local** trước khi xoá — segment chỉ được xoá local **sau khi đã upload**) + `retention.ms`/`retention.bytes` (tổng, áp cho **remote**). Nếu không đặt `local.*` thì lấy theo `retention.*`.
- **Hạn chế**: **không hỗ trợ compacted topic**; phải tắt trên mọi topic trước khi tắt ở broker; admin action cần client ≥ 3.0; segment thiếu producer snapshot (topic tạo trước 2.8) không hỗ trợ. Trước 3.9 không tắt được ở mức topic; từ KRaft: `remote.log.copy.disable=true` (read-only remote, nhớ set `local.retention.*=-2`) hoặc `remote.storage.enable=false,remote.log.delete.on.disable=true` (xoá hết remote).
- Metric: `RemoteCopyBytesPerSec`, `RemoteCopyLagBytes/Segments`, `RemoteFetchBytesPerSec`, `RemoteFetchErrorsPerSec`, `RemoteDeleteLagSegments`, `RemoteLogSizeBytes`, `RemoteLogReaderAvgIdlePercent`, `remote-fetch-throttle-time-avg`.
- Kafka 4.0 (KIP-1030): `remote.log.manager.copier.thread.pool.size` và `expiration.thread.pool.size` mặc định **10** (bỏ -1); `remote.log.manager.thread.pool.size` **2**. Kafka 4.3: `remote.log.metadata.topic.min.isr` mặc định **2** (KIP-1235); `follower.fetch.last.tiered.offset.enable` (KIP-1023) cho follower mới bỏ qua data đã tier; `ListOffsets` có `EARLIEST_PENDING_UPLOAD_TIMESTAMP` (-6).
- Quy trình test nhanh: tạo topic `--config remote.storage.enable=true --config local.retention.ms=1000 --config retention.ms=3600000 --config segment.bytes=1048576`, bơm dữ liệu bằng `kafka-producer-perf-test.sh`, thấy file `.log/.index/.timeindex/.snapshot/.leader_epoch_checkpoint` xuất hiện ở thư mục remote; consumer `--from-beginning` vẫn đọc được offset 0 từ remote.
...
Then, after the active segment is rolled, the old segment should be moved to the remote storage and get deleted. This can be verified by checking the remote log directory configured above:
...
### KIP-405 design summary (tổng hợp từ docs, không crawl được cwiki)

- **Motivation**: Kafka storage scales with broker count; long retention forces adding brokers only for disk; recovery/rebalance time grows with data on local disks. Tiered storage decouples storage from compute.
- **Components**: `RemoteLogManager` (broker-side thread pool: copier tasks upload rolled segments + indexes, expiration tasks delete remote segments past `retention.ms`), `RemoteStorageManager` (pluggable, put/fetch/delete segment data + indexes), `RemoteLogMetadataManager` (pluggable; default `TopicBasedRemoteLogMetadataManager` on `__remote_log_metadata`).
- **Fetch path**: consumer fetch for offsets no longer local is served via `RemoteLogReader` thread pool (metrics `RemoteLogReaderTaskQueueSize`, `RemoteLogReaderAvgIdlePercent`); delayed remote fetches can expire (`DelayedRemoteFetchMetrics,name=ExpiresPerSec`).
- **Retention**: `local.retention.*` applies to the local copy (only after upload), `retention.*` applies to the overall (remote) log; `log.retention.check.interval.ms` (default 300000) drives both.
- **Non-goals / limits**: only the active and un-uploaded segments live locally; compacted topics unsupported; JBOD supported since 3.7 but recommended to test; tiered storage GA declared in Kafka 3.9.
