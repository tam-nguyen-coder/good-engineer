...
>
> **Điều hướng:** [⬅️ Tuần 9](../week-09/README.md) · [🏠 Kế hoạch tổng](../../KAFKA-STUDY-PLAN.md) · 🎓 Đăng ký & thi
...
- **Review 100% câu sai** của mọi mock và **viết file phân tích** trong `KAFKA/questions/` theo đúng format 6 mục của repo (`aws-saa-c03-analysis-format.md`).
...
- **Đọc trôi chảy** toàn bộ **bảng số [§6](../../KAFKA-STUDY-PLAN.md#6-những-con-số-phải-thuộc-lòng)** và **bảng phản xạ [§7](../../KAFKA-STUDY-PLAN.md#7-bảng-phản-xạ-keyword--đáp-án)** của Kế hoạch tổng — không cần tra cứu.
...
| Ngày                       | Việc chính                                                                                                                                                  |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Ngày 1**           | 🎯**FULL MOCK #2** (60 câu, canh giờ 90') → **Review 100% câu sai** + viết file phân tích 6 mục                                                 |
...
| **Ngày 3**           | 🎯**FULL MOCK #3** (canh giờ 90') → Review 100% câu sai + viết file phân tích                                                                            |
| **Ngày 4**           | **Capstone bước 4–7** (Streams join+window EOS, Connect sink+DLQ, kill broker/instance, replay) + đọc lại **bảng phản xạ §7**                       |
| **Ngày 5**           | 🎯**FULL MOCK #4** *(nếu chưa đủ 3 bài ≥80%)* → Review. Nếu đã đủ → tự viết lại **cram sheet 1 trang** từ trí nhớ                            |
...
2. Với mỗi câu đáng nhớ, **viết một file phân tích** trong `KAFKA/questions/` (đặt tên `KAFKA-NNNN.md`) theo **format 6 mục** của `aws-saa-c03-analysis-format.md`:
...
| 2 | ✅ Đã**review hết 100% câu sai** và viết file phân tích 6 mục cho câu đáng nhớ trong `KAFKA/questions/`       |
...
- **Ôn tổng:** cram sheet 60 fact ở trên + **bảng số [§6](../../KAFKA-STUDY-PLAN.md#6-những-con-số-phải-thuộc-lòng)** + **bảng phản xạ [§7](../../KAFKA-STUDY-PLAN.md#7-bảng-phản-xạ-keyword--đáp-án)** của Kế hoạch tổng.
- **Sổ câu sai:** file phân tích 6 mục trong `KAFKA/questions/` (workflow giống SAA-C03).
...
- [ ] **Review 100% câu sai** + viết file phân tích 6 mục trong `KAFKA/questions/`
