...
>
> **Điều hướng:** [⬅️ Tuần 7](week-07.md) · [🏠 Kế hoạch tổng](../DVA-C02-STUDY-PLAN.md) · [Tuần 9 ➡️](week-09.md)
...
- `CodeCommit`: Git repo có quản lý (nay ít mới, nhưng vẫn ra đề). Trigger được sự kiện.
...
1. Stage **Source**: `CodeCommit` (hoặc GitHub qua **CodeConnections/CodeStar**) → thay đổi commit trigger qua **EventBridge**/webhook.
...
| `Blue/Green` | dựng env mới rồi **swap URL/CNAME** |
...
| `CodePipeline` | stage Source/Build/Deploy/Approval; artifact ở **S3**; trigger **EventBridge**/webhook; GitHub qua **CodeConnections/CodeStar** |
...
- **Beanstalk muốn zero-downtime + KHÔNG đụng instance cũ → policy nào?**
  **Đáp án gọn:** **`Immutable`** (tạo instance mới hoàn toàn, rollback dễ). (Blue/Green cũng zero-downtime nhưng qua swap CNAME.)
