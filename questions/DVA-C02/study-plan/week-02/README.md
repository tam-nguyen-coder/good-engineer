...
>
> **Điều hướng:** [⬅️ Tuần 1](../week-01/README.md) · [🏠 Kế hoạch tổng](../../DVA-C02-STUDY-PLAN.md) · [Tuần 3 ➡️](../week-03/README.md)
...
- **SnapStart** — giảm cold start (best-effort) cho `Java 11+`, `Python 3.12+`, `.NET 8+`; chỉ dùng trên **published version/alias** (không `$LATEST`); **không dùng chung provisioned concurrency**; không hỗ trợ `/tmp` > 512 MB.
...
| SnapStart | Java 11+/Python 3.12+/.NET 8+; chỉ version+alias; best-effort; ≠ provisioned concurrency |
| Payload sync | **6 MB** (`RequestResponse`) |
| Response streaming | tới **200 MB** (cách vượt cap 6 MB của sync) |
...
- Thấy "giảm **cold start**" → có 2 lựa chọn: **provisioned concurrency** (đảm bảo, có phí giữ ấm) hoặc **SnapStart** (Java/Python/.NET, miễn phí, best-effort, chỉ version/alias) — chọn theo runtime & chi phí.
...
| "giảm cold start cho Java/Python/.NET, không muốn trả phí giữ ấm liên tục" | **SnapStart** |
