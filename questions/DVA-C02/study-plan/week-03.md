...
>
> **Điều hướng:** [⬅️ Tuần 2](week-02.md) · [🏠 Kế hoạch tổng](../DVA-C02-STUDY-PLAN.md) · [Tuần 4 ➡️](week-04.md)
...
- **Transactional read = 2× RCU**; **transactional write = 2× WCU**.
...
| Giới hạn | Nhiều | **Tối đa 5 LSI/bảng** |
...
- **`DAX` (DynamoDB Accelerator):** cache **độ trễ micro-giây**, chạy **in-VPC**, **write-through**, chủ yếu tăng tốc **read** (đặc biệt read-heavy, repeated reads). Không thay thế cache cho aggregation phức tạp; không giúp write.
- **Batch & TTL & PartiQL:**
  ...
  - **TTL**: dựa trên **attribute dạng epoch (giây)**; item hết hạn bị xoá **trong khoảng ~48 giờ** sau đó — **KHÔNG tức thì**, không tính phí xoá. Dùng cho session, log tạm.
...
| Transaction | read = **2× RCU**; write = **2× WCU** |
...
| `DAX` | cache **micro-giây**, in-VPC, **write-through**, tăng tốc **read** |
| Throttling | `ProvisionedThroughputExceededException` → SDK retry **exponential backoff** |
| TTL | attribute **epoch**; xoá **~48 giờ** sau khi hết hạn (không tức thì) |
...
- Thấy **"dữ liệu tự hết hạn"** → **TTL**, nhưng nhớ xoá **không tức thì (~48h)** — nếu đề cần chính xác thời điểm thì phải lọc bằng expression.
...
| "tự xoá dữ liệu hết hạn" | **TTL** (~48h, không tức thì) |
