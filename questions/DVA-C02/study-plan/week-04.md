...
>
> **Điều hướng:** [⬅️ Tuần 3](week-03.md) · [🏠 Kế hoạch tổng](../DVA-C02-STUDY-PLAN.md) · [Tuần 5 ➡️](week-05.md)
...
- **Integration timeout tối đa: 29 giây** (khoảng 50 ms – 29 s) — nếu backend chạy lâu hơn sẽ bị cắt.
...
2. **`presigned URL`** — sinh URL tạm có hạn cho cả upload (PUT) và download (GET):
...
aws s3presign ... # hoặc dùng SDK: s3.generate_presigned_url('put_object', ...)
```
...
|------|-------------|-------------|
| `SSE-S3` | AWS quản lý (AES-256) | Mặc định, đơn giản, không audit theo khoá |
...
| Fact | Số / Giá trị |
|------|--------------|
| Integration timeout tối đa của `API Gateway` | **29 giây** (≈ 50 ms – 29 s) |
...
| `presigned URL` kế thừa quyền của | **identity đã tạo ra nó** (và có hạn thời gian) |
...
- Thấy "backend chạy 45 giây rồi API trả lỗi" → nhớ **timeout 29 giây** là cứng; giải pháp là async (trả 202 + xử lý nền), không phải tăng timeout.
...
2. `presigned URL` kế thừa quyền của ai?
   **Đáp án gọn:** Của **identity đã tạo ra URL** đó; URL có hạn thời gian, ai cầm cũng dùng được trong quyền đó cho tới khi hết hạn.
...
5. Integration timeout tối đa của `API Gateway` là bao nhiêu?
   **Đáp án gọn:** **29 giây** (khoảng 50 ms – 29 s).
